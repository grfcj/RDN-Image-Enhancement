...
class RDN(object):
    ...
    '''def PS(self, X, r):
        ...
        Xc = tf.split(X, 1, 1)
        if self.is_train:
            X = tf.concat([self._phase_shift(x, r) for x in Xc], 1)  # Do the concat RGB
        else:
            X = tf.concat([self._phase_shift_test(x, r) for x in Xc], 1)  # Do the concat RGB
        return X'''
    ...
    def train(self, config):
        ...
        data_dir = get_data_dir(config.is_train)
        ...
        summary_writer = tf.summary.FileWriter((os.path.join(config.checkpoint_dir, self.model_name, "log")),
                                               self.sess.graph)
        ...
        for ep in range(epoch_start, config.epoch):
            ...
            for idx in range(batch_start, batch_num):
                ...
                if counter % 10 == 0:
                    print("Epoch: [%4d], batch: [%6d/%6d], loss: [%.8f], lr: [%.6f], step: [%d]" % (
    ...
    def test(self, config):
        ...
        num=200
        results=np.zeros([num, input_.shape[1], input_.shape[2], input_.shape[3]])
        for i in range(num):
            _input_=input_[i,:,:,:]
            ...
            tf.global_variables_initializer().run(session=self.sess)
            self.load(config.checkpoint_dir, restore=True)
            result = self.sess.run([self.pred], feed_dict={self.images: _input_})
